...
public class InitController implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (restaurants.findByName("City Pizza").isEmpty())
            restaurants.save(citypizza);
        if (restaurants.findByName("Dominos Pizza").isEmpty())
            restaurants.save(dominos);
        if (restaurants.findByName("Pizerria Allora").isEmpty())
            restaurants.save(allora);
        if (pizzas.findByName("Carbonara").isEmpty())
            pizzas.save(new Pizza("Carbonara", restaurants.findByName("City Pizza").get(), "Delicious pizza with cheese, mashrooms, onions & carbonara sause",
                5.6, 7.8, 12.3,
                "https://img.taste.com.au/6MQTA3rp/taste/2016/11/carbonara-pizza-our-most-recent-recipes-of-the-day-104195-2.jpeg"));
        if (pizzas.findByName("Marinara").isEmpty())
            pizzas.save(new Pizza("Marinara", restaurants.findByName("City Pizza").get(), "Tomato Sauce, Garlic, Oregano & Basil. Vegan.",
                4.3, 6.2, 10.6,
                "https://flipdish.imgix.net/m10fmGwpZOWRKbrntJ4IuDhW7D0.jpg?auto=format&auto=compress&fit=crop&w=767&h=460&dpr=1"));
        if (pizzas.findByName("Anchovies").isEmpty())
            pizzas.save(new Pizza("Anchovies", restaurants.findByName("City Pizza").get(), "Anchovies, Fresh Chilies, Garlic, Capers, Mozzarella, Parsley & Tomato Sauce.",
                -1, 6.8, -1,
                "https://flipdish.imgix.net/XX17jEFwVvjLYaLvmAHO8Vw3Gl4.jpg?auto=format&auto=compress&fit=crop&w=767&h=460&dpr=1"));
        if (pizzas.findByName("Margarita").isEmpty())
            pizzas.save(new Pizza("Margarita", restaurants.findByName("Dominos Pizza").get(), "Mozzarella, Parmesan, Tomato Sauce, Olive Oil & Basil.",
                4.6, 6.9, 11.3,
                "https://flipdish.imgix.net/cOpSOzJIDhRn0xJk10gIaDtfMc.jpg?auto=format&auto=compress&fit=crop&w=767&h=460&dpr=1"));
        if (pizzas.findByName("Salami").isEmpty())
            pizzas.save(new Pizza("Salami", restaurants.findByName("Dominos Pizza").get(), "",
                4.6, -1, 11.3,
        ...
        if (pizzas.findByName("Prosciutto").isEmpty())
            pizzas.save(new Pizza("Prosciutto", restaurants.findByName("Pizerria Allora").get(), "With cheese and ham",
                -1, 7.1, 10.2,
        ...
        if (pizzas.findByName("Pizza Chicken").isEmpty())
            pizzas.save(new Pizza("Pizza Chicken", restaurants.findByName("Pizerria Allora").get(), "with cheese, chicken, onions & corn",
                5.2, 7.8, 10.9,
